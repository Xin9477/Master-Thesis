...
if __name__ == '__main__':
    ...
    # 加载微调后的模型和 tokenizer
    model_path = '/tmp/hongloumeng/hf_compatible_fenleiFTed_2'
    tokenizer_path = '/tmp/hongloumeng/hf_compatible_fenleiFTed_2'
    ...
    # 测试集范围
    y_test_range = file_list_y[16:32]  # Y-dataset 第17到32章
    n_test_range = file_list_n[8:16]   # N-dataset 第89到96章
    ...
    # 输出文件路径
    performance_file = '/tmp/hongloumeng/trained_bert_performance2.txt'
    percentage_file = '/tmp/hongloumeng/trained_bert_percentage2.txt'

    with open(performance_file, 'w') as perf_f, open(percentage_file, 'w') as perc_f:
        datasets = [
            (y_test_range, folder_path_y, 17),  # Y-dataset 第17到32章
            (n_test_range, folder_path_n, 89)   # N-dataset 第89到96章
        ]
